...
import type { User, Artwork, Order, Review, Message } from "./types";
...
export const logOut = async (router: AppRouterInstance): Promise<void> => {
  await axios.get(`${server_url}/log_out`).then(() => {
    router.push("/");
  }).catch(console.log);
};
...
export const getIsAdmin = async (): Promise<{ data: { is_admin: boolean } }> => {
  return axios.get(`${server_url}/${admin_url}/is_admin`);
};

export const updateUserData = async (field_name: string, value: string): Promise<{ data: User }> => {
...
export const getArtworkSearchResults = async (objects: any, pageNumber: number): Promise<Artwork[]> => {
  ...
  return axios
    .get(
      `${server_url}/search_artworks${queries.length ? "?" + queries : ""}${objects.n ? "&offset=" + objects.n * (pageNumber - 1) : ""}`,
    )
    .then(res => res.data)
    .catch(error => {
      console.log(error);
...
export const getDataOfArtworks = async (shoppingCart: any): Promise<Artwork[]> => {
...
export const addNewArtwork = async (artwork: ArtworkSent): Promise<{ data: number }> => {
  return await axios.post(`${server_url}/${admin_url}/add_new_artwork`, { artwork });
};
...
export const removeFromShoppingList = async (artwork_id: number): Promise<any> => {
  await axios.post(
...
export const increaseShoppingListItemQuantity = async (artwork_id: number): Promise<any> => {
  await axios.post(
...
export const decreaseShoppingListItemQuantity = async (artwork_id: number): Promise<any> => {
  await axios.post(
...
export const replaceSavedShoppingCart = async (shopping_cart: any): Promise<void> => {
...
export const removeFromWishlisted = async (artwork_id: number): Promise<any> => {
...
export const getOrderHistory = async (user_id: number): Promise<Order[]> => {
  const res = await axios.post(`${server_url}/${admin_url}/get_orders_of_user`, {
    user_id,
  });
...
export const addNewThumbnail = async (artwork_id: number, thumbnail: Blob): Promise<any> => {
  const formData = new FormData();
  formData.append('thumbnail', thumbnail);
  await axios.post(`${server_url}/${admin_url}/thumbnail?artwork_id=${artwork_id}`, formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
};


export const addNewOtherPictures = (artwork_id: number, other_pictures: Blob[]): Promise<any[]> => {
  return Promise.all(
    other_pictures.map(picture => addNewOtherPicture(artwork_id, picture))
  );
};

export const addNewOtherPicture = async (artwork_id: number, picture: Blob): Promise<any> => {
  const formData = new FormData();
  formData.append('picture', picture);
  await axios.post(`${server_url}/${admin_url}/picture?artwork_id=${artwork_id}`, formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
};

export const removePicture = async (artwork_id: number, file_name: string): Promise<any> => {
  await axios.post(`${server_url}/${admin_url}/remove_picture`, { artwork_id, file_name });
};

export const replaceThumbnail = async (artwork_id: number, thumbnail: Blob): Promise<any> => {
  const formData = new FormData();
  formData.append('thumbnail', thumbnail);
  await axios.post(`${server_url}/${admin_url}/replace_thumbnail?artwork_id=${artwork_id}`, formData, {
    headers: { 'Content-Type': 'multipart/form-data' },
  });
};